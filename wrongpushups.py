...
class VideoThread(QThread):
    ...
    def run(self):
        location = 'Videos/pushups/b.mp4'
        cap = cv2.VideoCapture(location)
        print(location)
        # newlocation = 'Finished/' + self.workout +'_' + self.file + '.mp4'
        ...
        while True:
            try:
                    ...
                    if int(spineAngle) > 120 and int(heelAngle) in range(60, 90) and int(rightLegAngle) > 150 and int(
                        ...
                        if startPosition:
                            ...
                            if int(count) > 0:
                                ...
                                if hipAngle < 155:
                                    hipCorrection = f"Hips are not straight"
                                ...
                                for key, value in maxElbowUpAngle.items():
                                    ...
                                    if max(value) < 150:
                                        print(max(value))
                                        elbowUpCorrection = f"Straighten your arms"
                            ...
                            for x, correction in enumerate(corrections):
                                cv2.putText(img, correction, (10, (int(x) * 50) + 150), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            red, 2)
                            print(correction)
                            ...
                            cv2.putText(img, str(int(count)), (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 2,
                                        blue, 10)
                        # get the screenshot
                        # cv2.imwrite(f"Finished/{self.workout}/{self.file}.jpg", img)
                ...
                if success:
                    ...
                    #
                    # with open(f"Finished/{self.workout}/{self.file}.txt", "w") as file:
                    #     for items in text:
                    #         file.write(f"{items}\n")

            except:
                # print("error")
                # cv2.imwrite("Finished/sample.jpg", img)
                cap.release()
...
class PushUps(object):
    ...
    #     MainWindow.setWindowTitle(_translate("MainWindow", "PushUps"))
